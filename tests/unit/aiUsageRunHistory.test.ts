...
describe('AI spend run history',()=>{
  it('keeps aggregate spend separate and groups requested research runs with useful labels',async()=>{
    ...
    try{
      ...
      await recordAiUsage({DB:db},'owner',{kind:'champagne_extraction',runId:'champagne-run',targetId:'w1',eventId:'champagne-1',model:'gemini-3.1-flash-lite',tier:'flex',requests:1,promptTokens:450,outputTokens:180});
      ...
      expect(summary.kinds.find(kind=>kind.kind==='champagne_extraction')).toMatchObject({label:'Champagne details extraction',runs:1,unit:'run'});
      ...
      expect((await usageRunHistory(db,'owner',DEFAULT_RATES,'champagne_extraction',30)).runs[0]).toMatchObject({
        runId:'champagne-run',targetId:'w1',targetLabel:'Domaine Test · 2020 · Les Suchots',requests:1,searchQueries:0
      });
