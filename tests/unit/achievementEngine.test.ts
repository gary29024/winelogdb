...
describe('achievement engine',()=>{
  ...
  it('retroactively counts stable producer identities in MICHELIN Grape collections',()=>{
    const twoGrapes=getAchievementDefinition('michelin-grapes-burgundy-2026-two');
    expect(twoGrapes).not.toBeNull();
    const result=buildAchievementProgress(twoGrapes!,registry,wines);
    expect(result).toMatchObject({completed:1,total:20,percent:5});
...
describe('starter achievement definitions',()=>{
  it('ships twenty curated launch collections with stable item counts',()=>{
    expect(achievementDefinitions.map(item=>[item.id,item.items.length])).toEqual([
      ...
      ['michelin-grapes-burgundy-2026-three',9],
      ['michelin-grapes-burgundy-2026-two',20],
      ['michelin-grapes-burgundy-2026-one',33],
      ['michelin-grapes-burgundy-2026-selected',32]
    ]);
  });
  ...
  it('versions MICHELIN Grape collections by series, region, edition and tier',()=>{
    const michelin=achievementDefinitions.filter(item=>item.series?.id==='michelin-grapes');
    expect(michelin).toHaveLength(4);
