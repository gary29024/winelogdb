...
const measured={ok:true as const,result:{bottle:box,label:null,confidence:.9},requestId:'r-1',durationMs:120,finishReason:null,wineCount:1,owner:'owner-1'};
...
describe('measuring one photograph',()=>{
  it('stores what it measured, so the same card never pays twice',async()=>{
    ...
    expect(stub.writes()[0].args).toEqual([serializeBottleFrame({bottle:box,label:null}),'owner-1','i-1']);
  });

  it('records a photograph with no bottle in it, rather than leaving it to be asked again',async()=>{
    const {measureBottleFrame}=await handler({...measured,result:{bottle:null,label:null,confidence:0},wineCount:0});
    const stub=createD1Stub(()=>({changes:1}));
    await measureBottleFrame(new Request('https://x/api/bottle-frames/i-2',{method:'POST'}),{DB:stub.db} as never,'i-2');
    expect(stub.writes()[0].args[0]).toBe('{"bottle":null,"label":null}');
  });
...
describe('the spec it runs under',()=>{
  it('asks for boxes only, cheaply, and never escalates',async()=>{
    ...
    expect(bottleFrameSpec.escalationReasons({bottle:null,label:null,confidence:0})).toEqual([]);
    expect(bottleFrameSpec.maxOutputTokens).toBeLessThanOrEqual(1024);
    expect(bottleFrameSpec.wineCount({bottle:box,label:null,confidence:1}),'a measured bottle is the metered unit').toBe(1);
    expect(bottleFrameSpec.wineCount({bottle:null,label:null,confidence:0}),'nothing found is nothing billed as a unit').toBe(0);
  });
  ...
  it('tells the model to leave everything that is not the main bottle alone',async()=>{
    ...
    expect(prompt,'a tilted bottle is the common case, not the exception').toMatch(/tilted/i);
    expect(prompt,'and an empty answer has to be allowed or it will invent one').toMatch(/return null for both boxes/);
  });
});
