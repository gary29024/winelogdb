...
describe('hybrid Journal search',()=>{
  it('unions semantic candidates with FTS and ranks semantic results first by default',async()=>{
    ...
    expect(page!.sql).toContain('LEFT JOIN json_each(?) semantic_rank ON CAST(semantic_rank.value AS TEXT)=w.id');
    expect(page!.sql).toContain('COALESCE(CAST(semantic_rank.key AS INTEGER), 2)');
  ...
  it('honours an explicit user sort instead of semantic ranking',async()=>{
    ...
    expect(page!.sql).not.toContain('LEFT JOIN json_each(?) semantic_rank');
  });
});
