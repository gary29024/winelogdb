...
| One-off LWIN identity repair | AI Gateway / Vertex Flex | `gemini-3.1-flash-lite` | Owner-triggered second pass only. Local LWIN candidates are narrowed first; the model may select only a supplied LWIN7 or reject all. |\n| Champagne photo extraction | Vertex Flex through AI Gateway, or Developer API Batch | `gemini-3.1-flash-lite` | Intentionally remains on 3.1 Flash Lite. |
