...
The browser uses the same quote, byte-identical multipart submission, account protection and idempotent network retry as other AI actions. If acceptance arrives before the extraction row, it follows the accepted operation's bound run ID instead of displaying a previous extraction. The Worker validates ownership, Champagne eligibility and the chosen saved photos before reservation, before provider submission and before saving results. The extraction row and operation run ID are linked in one D1 batch before dispatch. Successful results settle the operation; ordinary failures, expiry and deleted bottles release it. An ambiguous provider submission remains in review, deliberately preventing automatic paid resubmission.

Provider routing is unchanged: Gemini 3.1 Flash Lite, Vertex AI Gateway Flex or native Gemini Batch. Native Batch now carries the same durable provider context as other research jobs. Temporary polling/dispatch errors resume the existing batch, not a new paid submission. Status reads can recover polling with the original reservation. Already-submitted batches continue polling after photo deletion or wine edits, recording their paid usage before rejecting results whose inputs changed. AI usage remains attributed once per run, including unusable provider answers.
