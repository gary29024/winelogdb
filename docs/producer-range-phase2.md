...
After deploying the diagnostics, retry the producer range refresh and search the `winelogdb` Worker logs for `workers_ai_failed`, then match the new run's `requestId`. This event includes `failurePhase` (inference, response, or usage_recording), `failureKind`, `elapsedMs`, `timeoutMs`, model, and numeric `providerCode`/`httpStatus` when available. `local_timeout` means the application's 75-second deadline; `empty_response` means inference returned without usable text. Cloudflare code `3036` means daily free neurons exhausted, `3040` means capacity unavailable, and `3007` means provider timeout. Unknown provider messages are withheld rather than logging prompts or credentials. Older failures cannot be reconstructed from these new fields.

On the Vertex gateway path, Batch submission stores a local job; its first poll executes Vertex Flex requests. All entries, concurrency waves, retry delays, and response-body reads share an 11-minute execution budget, below the 12-minute stale-job threshold and Cloudflare's 15-minute queue-consumer limit. Each attempt gets at most 10 minutes and a retry only receives the remaining shared time. Timeouts become stored per-entry errors so the existing research recovery can schedule the next batch in a new queue delivery. An envelope marked `JOB_STATE_SUCCEEDED` means execution results were saved; individual entries can still contain errors. `vertex-flex-attempt` logs report start/response/failure/budget exhaustion with job ID, research request ID where available, model, timing, and safe failure categories. Queue start logs include delivery attempt and poll count to distinguish redelivery from status polling. A missing AI-spend entry does not establish that no provider request was sent; usage is recorded when results return.
