...
When the wine has no recorded classification and names a known Premier Cru of its own appellation, or a Chablis Grand Cru climat, the village link is withheld. Only that appellation's crus count: La Romanée is a Gevrey-Chambertin Premier Cru and a Vosne-Romanée Grand Cru, and Les Perrières in Meursault says nothing about a Saint-Romain wine. A recorded `village` classification remains authoritative; a plain village name without a known cru can still link when the classification is empty.

`burgundyAtlasUnmappedPremierCruNames.json` covers all 36 Atlas Premier Cru records without map presentations: 30 in Chablis, two in Givry, one in Pouilly-Loché and three in Pouilly-Vinzelles. These names only prevent an unsupported village link; they do not enable specific plot links or infer a wine's classification. Atlas's identity-only records do not publish a parent area, so their appellations were checked separately on 2026-09-23 against the [INAO Chablis climat list](https://extranet.inao.gouv.fr/fichier/CDC-Chablis-250623-PNO.pdf), [BIVB Chablis map](https://www.chablis-wines.com/gallery_files/site/25717/25787/73290.pdf), [BIVB Givry appellation sheet](https://www.bourgogne-wines.com/wine-and-terroir/bourgogne-and-its-appellations/gallery_files/site/321/402/79294/79373.pdf) and [Pouilly-Loché / Pouilly-Vinzelles appellation announcement](https://www.bourgogne-wines.com/press/gallery_files/site/289/1910/78880.pdf). The Premier Cru refresh stops if an unmapped Atlas identity or name lacks this review. Both refresh scripts require the complete 32-shard registry set.
...
The mapping is shared static frontend data. Matching reuses compiled patterns for the static place names across fields and renders. There are no migrations, database reads, AI calls, embeds or automatic requests to Atlas. Navigation sends only the public Atlas destination; `noopener noreferrer` omits the WineLog referrer and opener.
