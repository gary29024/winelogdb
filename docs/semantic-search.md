...
That command builds the app, applies D1 migrations, then deploys the Worker. Migration `0057_semantic_search_embeddings.sql` creates the local vector cache. Migration `0061_semantic_query_cache.sql` adds a short-lived ranked-query cache plus a semantic-index revision row used to invalidate those cached rankings safely.
...
A 1,024-dimensional Float32 vector is 4,096 bytes before row/query overhead. Reading the full D1 vector set is therefore roughly 0.39 MiB for 100 wines and 3.9 MiB for 1,000 wines on a semantic cache miss. Repeating the same Smart search while its 30-minute ranked-result cache is valid avoids both that vector scan and another query-embedding request. Around 1,000 wines — or earlier if semantic-search latency or D1 rows read become material even with this cache — is the point to reassess and move the storage/ranking layer to Vectorize. That migration would not require changing the Journal UI or embedding provider.
...
This means a user typing `floral elegant Burgundy` still sees normal literal/metadata results automatically. No query embedding is spent merely because the user paused while composing the description. The URL records this distinction as `semantic=0` for the live lexical version and a positive semantic attempt number after Smart search. The first attempt is `semantic=1`; choosing **Smart search again** increments that number so the same text produces a real Journal refetch while background indexing catches up.

The UI does not disable Smart search or label the first semantic attempt as definitively complete. It keeps **Smart search again** available so a cold or partially warmed index can be retried. If the semantic index has not changed, a repeated attempt may now be satisfied by the valid ranked-result cache rather than spending another embedding request. Once background indexing changes the semantic index revision, the old ranking is automatically ineligible and the next attempt recomputes against the newer vectors. It intentionally avoids a permanent “indexing” disclaimer because once the index is warm that message would be false; if richer index-state feedback is needed later, it should be driven by explicit server state rather than inferred from `semantic>0`.
...
Document indexing never blocks the Journal response. An explicit Smart search schedules bounded cache warm-up through `waitUntil`; on a completely cold vector cache the current request simply uses the existing lexical result while vectors are built in the background. Once cached candidates exist, a semantic cache miss waits only for the one query embedding.

After ranking, the Worker stores the candidate IDs in `wine_semantic_query_cache`, keyed by owner, embedding model and an NFKC/whitespace-normalized query. The cache lives for 30 minutes. It deliberately does not include the Journal `semantic` attempt counter in the key: opening a wine and returning to the same Journal URL, or pressing Smart search again without any index change, can reuse the same ranking without another embedding provider call.

Every successful document-embedding refresh advances `wine_semantic_index_state.revision` once per refresh batch. Query-cache rows record the revision they were ranked against. A background refresh therefore makes older rows immediately ineligible even if a query request races with that refresh. Wine deletion cascades its stored vector and a database trigger also advances the relevant model revision. Expired or superseded query-cache rows are cleaned opportunistically when a new ranking is stored.

The vector cache records the wine's `updated_at`. An unchanged wine is never re-embedded. If a wine changes, its previous vector remains searchable until the background refresh replaces it, so an edit does not make that wine temporarily disappear from semantic results. Deleted wines are excluded by the live-wine join and their cached embedding is removed by the foreign-key cascade.

Embedding model calls are recorded in the app AI-usage ledger as **Smart search** (`search_embedding`). Document-index calls count the number of wines embedded; query embeddings count as requests but do not pretend to index another wine. A ranked-result cache hit records no new embedding request because no embedding provider is called. The current embedding response shapes do not expose exact billed token/neuron usage, so provider dashboards remain authoritative for exact embedding compute spend.

If the embedding service is unavailable, misconfigured or over quota, the Worker logs the failure and falls through to the existing FTS Journal search rather than failing the page. Query-cache read/write failures are also treated as optimization failures only: they do not make an otherwise valid semantic search fail.
...
4. Open one of the returned wines, then go back to Journal — the same semantic result set should return without another query-embedding request while the cache remains valid.
5. If the first set looks incomplete because the index was still warming, tap **Smart search again** after the background refresh — the index revision change invalidates the old ranking and the query is recomputed once against the warmer index.
6. Edit the text again — after the next 300 ms pause it should return to the cheap lexical mode until Smart search is explicitly chosen again.
7. While a search debounce is pending, change another filter such as Style — when the search settles, both the text and the newer filter should remain in the URL/results.
