...
export const bottleFrameSpec:RecognitionModeSpec<BottleFrameResult>={
  ...
  prompt:()=>`This photograph shows a wine bottle, usually held up by hand. Return only where things are, using normalized image coordinates from 0 to 1000 (xMin,yMin,xMax,yMax). "bottle": a tight box around the visible glass of the single main bottle - the one held up or nearest the camera - from the top of its neck or capsule to the base, or to the bottom edge of the photo where the base is out of shot. Follow the bottle even when it is tilted: the box is the upright rectangle that contains it. "label": a tight box around that bottle's main front paper label only, not the neck label and not the capsule. Ignore other bottles, glasses, hands, plates and anything in the background. If there is no wine bottle in this photograph, or the main bottle cannot be told apart from what is behind it, return null for both boxes rather than guessing. "confidence" is 0 to 1 for how sure you are of the bottle box. Return ONLY valid JSON: {"bottle":{"xMin":0,"yMin":0,"xMax":1000,"yMax":1000},"label":null,"confidence":0.0}. Do not use Markdown fences.`
};
