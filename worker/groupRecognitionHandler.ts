...
export async function handleGroupRecognitionRequest(request:Request,env:Bindings){
  ...
  const requestBody=JSON.stringify({contents,generationConfig:{responseMimeType:'application/json',responseJsonSchema:groupRecognitionResponseJsonSchema,maxOutputTokens:8192}});
  const schemaFreeBody=JSON.stringify({contents,generationConfig:{responseMimeType:'application/json',maxOutputTokens:8192}});
  console.log(JSON.stringify({event:'group-recognition-start',requestId,model:MODEL,inputBytes:file.size,inputType:file.type,inputName:file.name,outputMode:'json-schema'}));
  for(let attempt=1;attempt<=2;attempt++){
    ...
    try{
      ...
      // One photograph, however many bottles were on the table - which is
      // exactly why a group photo cannot be quoted per run.
      for(const [index,call] of [{model:MODEL,...usageOf(payload)},...(escalation.usage?[{model:RECOGNITION_ESCALATION_MODEL,...escalation.usage}]:[])].entries())
        await recordAiUsage(env,owner,{kind:'scan_group',runId:requestId,model:call.model,requests:1,units:index===0?result.wines.length:0,promptTokens:call.promptTokens,outputTokens:call.outputTokens});
