...
export async function handleGroupRecognitionRequest(request:Request,env:Bindings){
  ...
  let form:FormData;try{form=await request.formData()}catch{return json({error:'Could not read group recognition request',requestId},400,requestId)}
  const files=form.getAll('images').filter((x):x is File=>x instanceof File);
  try{validateBatch(files,{maxFiles:1,maxBytes:3*1024*1024,minDimension:300,maxDimension:2000})}catch(e){return json({error:(e as Error).message,requestId},400,requestId)}
  if(files.length!==1)return json({error:'Choose exactly one group photo',requestId},400,requestId);
  ...
  console.log(JSON.stringify({event:'group-recognition-start',requestId,model:MODEL,inputBytes:file.size}));
