...
import { readableWine } from '../../src/lib/research/readableWine';
...
export async function workKey(db:D1Database,user:string,path:string,request?:Request){
 ...
 if(wine){const row=await readableWine(db,user,wine[1]);if(!row)return null;const key=sharedSubjectKey(wineTargets(row).find(t=>t.scope==='wine_vintage')!);return key?`wine:${key}`:null}
