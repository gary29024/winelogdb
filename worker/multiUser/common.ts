...
export type PilotSettings={memberLimit:number;memberStorageBytes:number;totalStorageBytes:number;aiConcurrency:number;aiDailyOperations:number;aiDailyEmbeddingRequests:number;researchRunsPerWeek?:number;aiMonthlyBudgetUsd:number;aiUnitBudgetUsd:number;cloudflareWarningUsd:number;cloudflareStopUsd:number;cloudflareObservedUsd:number;cloudflareObservedMonth:string;allowOverages:boolean};
export async function settings(db:D1Database){const row=await db.prepare('SELECT value_json FROM pilot_settings WHERE id=1').first<{value_json:string}>();if(!row)throw new ApiError(503,'Owner must configure pilot budgets first');const value=JSON.parse(row.value_json) as PilotSettings;return {...value,researchRunsPerWeek:Number.isSafeInteger(value.researchRunsPerWeek)&&Number(value.researchRunsPerWeek)>0?Number(value.researchRunsPerWeek):2}}
