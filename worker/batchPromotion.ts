...
type RecognitionSummary=Record<string,unknown>&{locationName?:unknown};
type ConfirmedWineIdentity={producer:string;wine_name:string;vintage:number|null};
...
export function mergeConfirmedWineIdentity(recognition:RecognitionSummary,wine:ConfirmedWineIdentity):RecognitionSummary{
  return {...recognition,producer:wine.producer,wineName:wine.wine_name,vintage:wine.vintage};
}
...
export async function attachConfirmedItemWithMetadata(env:Env,owner:string,sessionId:string,itemId:string,wineId:string){
  ...
  const wine=await env.DB.prepare('SELECT producer,wine_name,vintage FROM wines WHERE owner_id=? AND id=?').bind(owner,wineId).first<ConfirmedWineIdentity>();
  if(!wine)throw new Error('Saved wine not found during batch confirmation');
