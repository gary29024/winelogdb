...
app.post('/api/producers/:id/unlink',async c=>{
  ...
  if(body.confirmation!=='UNLINK_PRODUCER'||!body.mergeId)return c.json({error:'Producer unlink requires an existing producer selection and explicit confirmation'},400);
...
app.delete('/api/producers/:id/hero-image',async c=>{
  cors(c);let owner:string;try{owner=await user(c)}catch{return c.json({error:'Unauthorized'},401)}
  const body=await c.req.json().catch(()=>({})) as {confirmation?:string};
  if(body.confirmation!=='REMOVE_PRODUCER_PHOTO')return c.json({error:'Removing a producer photo requires explicit confirmation'},400);
  try{return c.json(await rejectProducerHeroImage(c.env,owner,c.req.param('id')))}
  catch(e){const message=(e as Error).message||'Could not remove this photo';return c.json({error:message},message==='Producer not found'?404:400)}
});
