...
type Env=GeminiTransportBindings&{DB:D1Database;WINE_IMAGES:R2Bucket;REFERENCE_DATA:R2Bucket;RESEARCH_QUEUE:Queue<unknown>;AI_USAGE?:AnalyticsSink};
...
async function tryEscalatedBatchRecognition(env:Env,sessionId:string,itemId:string,body:string,primary:ReturnType<typeof parseRecognition>,reasons:string[],meter:RecognitionUsage){
  ...
  console.log(JSON.stringify({event:'vertex-flex-batch-recognition-escalation-start',sessionId,itemId,fromModel:RECOGNITION_MODEL,toModel:RECOGNITION_FLEX_ESCALATION_MODEL,reasons,primaryConfidence:primary.confidence}));
  try{
    const transport=await postGeminiGenerateContent(env,RECOGNITION_FLEX_ESCALATION_MODEL,body,controller.signal,{feature:'recognition',mode:'batch-escalation',session:sessionId,item:itemId,tier:'flex'},{serviceTier:'flex',serverTimeoutSeconds:600}),response=transport.response,provider=transport.provider;
    clearTimeout(timer);
    if(!response.ok){const raw=(await response.text().catch(()=>'' )).slice(0,2000);console.warn(JSON.stringify({event:'vertex-flex-batch-recognition-escalation-skipped',sessionId,itemId,model:RECOGNITION_FLEX_ESCALATION_MODEL,provider,status:response.status,reasons,error:errorMessage(raw,response.status)}));return {result:primary,used:false,trafficType:null}}
    const payload=await response.json() as GeminiResponse;
    meter.capture(RECOGNITION_FLEX_ESCALATION_MODEL,payload.usageMetadata);
    const candidate=payload.candidates?.[0],text=candidate?.content?.parts?.map(part=>part.text??'').join('')??'';
    if(!text)throw new Error('Vertex escalation returned an empty recognition');
    const escalated=parseRecognition(text),result=preferEscalatedRecognition(primary,escalated),used=result===escalated;
    console.log(JSON.stringify({event:'vertex-flex-batch-recognition-escalation-complete',sessionId,itemId,model:RECOGNITION_FLEX_ESCALATION_MODEL,provider,reasons,used,trafficType:payload.usageMetadata?.trafficType??null,primaryConfidence:primary.confidence,escalatedConfidence:escalated.confidence,latencyMs:Date.now()-startedAt,finishReason:candidate?.finishReason??null,promptTokens:payload.usageMetadata?.promptTokenCount??null,outputTokens:payload.usageMetadata?.candidatesTokenCount??null,thinkingTokens:payload.usageMetadata?.thoughtsTokenCount??null,totalTokens:payload.usageMetadata?.totalTokenCount??null}));
    return {result,used,trafficType:payload.usageMetadata?.trafficType??null};
  }catch(e){clearTimeout(timer);console.warn(JSON.stringify({event:'vertex-flex-batch-recognition-escalation-skipped',sessionId,itemId,model:RECOGNITION_FLEX_ESCALATION_MODEL,reasons,timedOut,latencyMs:Date.now()-startedAt,error:(e as Error).message||'Escalation failed'}));return {result:primary,used:false,trafficType:null}}
}
