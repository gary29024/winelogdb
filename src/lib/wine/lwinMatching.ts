...
import { regionWithin } from './referenceGeography';
...
export function compatibleLwinProduct(row:LwinReferenceProduct,input:LwinClues){
 ...
 if(place.region&&candidate.region&&!regionWithin(place.region,candidate.region,input.country??place.country,row.country??candidate.country)&&!regionWithin(candidate.region,place.region,row.country??candidate.country,input.country??place.country))return false;
 const pairs=[ [place.country,candidate.country],[colour,row.colour],[subtype,row.productSubtype],[type,row.productType],
