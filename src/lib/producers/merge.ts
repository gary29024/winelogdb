...
type ProducerRow={
  ...
  official_website_url?:string|null;hero_image_object_key?:string|null;hero_image_source_url?:string|null;
...
const producerColumns='id,canonical_name,match_key,home_country,home_region,home_locality,official_website_url,hero_image_object_key,hero_image_source_url,profile,catalog_json,sources_json,research_model,researched_at,created_at,updated_at';
...
export async function mergeProducerEntities(db:D1Database,owner:string,destinationId:string,sourceId:string){
  ...
  for(const row of producerResearch){
    statements.push(archiveStatement(db,owner,destinationId,{mergeId,originId:row.id,originName:row.canonical_name,type:'producer_record',payload:JSON.stringify({homeCountry:row.home_country,homeRegion:row.home_region,homeLocality:row.home_locality,officialWebsiteUrl:row.official_website_url??null,heroImageObjectKey:row.hero_image_object_key??null,heroImageSourceUrl:row.hero_image_source_url??null,profile:row.profile,catalog:parseJson(row.catalog_json,[])}),sources:row.sources_json,model:row.research_model,researchedAt:row.researched_at},now));
  }
  ...
  if(activeProducer){
    ...
    statements.push(db.prepare(`UPDATE producers SET home_country=?,home_region=?,home_locality=?,official_website_url=?,hero_image_object_key=?,hero_image_source_url=?,profile=?,catalog_json=?,sources_json=?,research_model=?,researched_at=?,updated_at=? WHERE owner_id=? AND id=?`).bind(activeProducer.home_country,activeProducer.home_region,activeProducer.home_locality,officialWebsite,mediaProducer.hero_image_object_key??null,mediaProducer.hero_image_source_url??null,activeProducer.profile,activeProducer.catalog_json,JSON.stringify(combined),activeProducer.research_model,activeProducer.researched_at,now,owner,destinationId));
  }
...
export async function unlinkProducerMerge(db:D1Database,owner:string,destinationId:string,mergeId:string){
  ...
  statements.push(db.prepare(`INSERT INTO producers(id,owner_id,canonical_name,match_key,home_country,home_region,home_locality,official_website_url,hero_image_object_key,hero_image_source_url,profile,catalog_json,sources_json,research_model,researched_at,created_at,updated_at)
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(source.id,owner,source.canonical_name,source.match_key,source.home_country,source.home_region,source.home_locality,source.official_website_url??null,source.hero_image_object_key??null,source.hero_image_source_url??null,source.profile,source.catalog_json,source.sources_json,source.research_model,source.researched_at,source.created_at||now,now));
  ...
  if(shouldRestorePreMerge(destination.updated_at,merge.merged_at)){
    statements.push(db.prepare(`UPDATE producers SET canonical_name=?,match_key=?,home_country=?,home_region=?,home_locality=?,official_website_url=?,hero_image_object_key=?,hero_image_source_url=?,profile=?,catalog_json=?,sources_json=?,research_model=?,researched_at=?,updated_at=? WHERE owner_id=? AND id=?`).bind(destinationBefore.canonical_name,destinationBefore.match_key,destinationBefore.home_country,destinationBefore.home_region,destinationBefore.home_locality,destinationBefore.official_website_url??null,destinationBefore.hero_image_object_key??null,destinationBefore.hero_image_source_url??null,destinationBefore.profile,destinationBefore.catalog_json,destinationBefore.sources_json,destinationBefore.research_model,destinationBefore.researched_at,now,owner,destinationId));
  }
