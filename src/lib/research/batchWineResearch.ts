...
const DEEP_SEARCH_OUTPUT_TOKENS=16384;
...
function researchTargets(wine:WineRow){return wineRowResearchTargets(wine)}

/**
 * How a wine without a vintage is researched. Nothing is said for a vintage
 * wine. A release with an edition, base year or disgorgement is researched as
 * that release, since editions of one non-vintage cuvée are different blends.
 * A known base year's growing season is researched as base-year context, never
 * as though the wine were a single vintage. A blend with no base year gets no
 * vintage section; its exact-wine research says which harvests it draws on
 * where sources state it.
 */
export function releaseResearchContext(wine:Pick<WineRow,'vintage'|'release_designation'|'sparkling_details_json'|'vintage_kind'>){
  if(wine.vintage!=null)return '';
  const details=parseJson<Record<string,unknown>>(wine.sparkling_details_json,{})??{};
  const edition=String(wine.release_designation??'').trim(),disgorgement=typeof details.disgorgement==='string'?details.disgorgement.trim():'';
  const baseVintage=typeof details.baseVintage==='number'&&Number.isFinite(details.baseVintage)?details.baseVintage:null;
  const kind=wine.vintage_kind==='multi_vintage'?'a multi-vintage blend':'a non-vintage wine';
  const release=[edition&&`edition/release "${edition}"`,baseVintage!=null&&`base vintage ${baseVintage}`,disgorgement&&`disgorged ${disgorgement}`].filter(Boolean).join(', ');
  const lines=[`This is ${kind}, not a single-vintage wine.${release?` Known release: ${release}.`:' No edition, base vintage or disgorgement is recorded.'}`];
  lines.push(release
    ?'- exact wine + vintage: research this specific release. Editions of one non-vintage cuvée are different blends, so prefer sources about this edition, base year or disgorgement. If sources describe only the cuvée in general, say so plainly, and never attribute another edition\'s blend, dosage, ageing or tasting notes to this one.'
    :'- exact wine + vintage: research this non-vintage cuvée as a whole, and say that the release is not identified.');
  lines.push('- For the blend: where sources state which harvest years and what share of reserve wines make up this release, describe that composition in winemakingTechniques; otherwise say it is not stated. Do not invent years.');
...
async function loadWine(db:D1Database,owner:string,wineId:string,credit?:ProviderAuthorization){
  ...
  const found=await readableWine<WineRow&{source_snapshot:string|null}>(db,owner,wineId,'w.vintage_kind,w.lwin7,w.identity_match_status,w.lwin_reference_json,w.producer,w.producer_id,w.cuvee_id,w.wine_name,w.vintage,w.country,w.region,w.appellation,w.wine_style,w.grapes_json,w.grape_blend_json,w.deep_search_json AS source_snapshot');if(!found)return null;
  const wine:ResearchWineRow<WineRow>=await researchWine(db,owner,found,found.source_snapshot);
...
export async function pollWineBatchResearch(env:Env,owner:string,wineId:string,requestId:string,jobId:string,pollCount:number){
  ...
  try{
    ...
    if(cut){
      const concise='the previous answer hit the output limit — keep every required field and its evidence, but make each field concise, remove repetition, and prioritize claims needed for the requested scope';
      feedback=Object.fromEntries(scopes.map(scope=>[scope,[concise]]));
    }
