...
export async function pollWineBatchResearch(env:Env,owner:string,wineId:string,requestId:string,jobId:string,pollCount:number){
  ...
  try{
    ...
    const provenance=buildDeepSearchProvenance(parsed.data,metadata),researched:DeepSearchResult={...parsed.data,provenance};
  ...
  await updateWineResearchRun(env.DB,owner,requestId,'complete','Gemini Batch Deep Search complete with claim-level evidence mapping','complete',job.attempt);log('log',{requestId,wineId,stage:'complete',attempt:job.attempt,model:job.model,scopes});
}
