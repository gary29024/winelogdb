...
export default function App(){return <BrowserRouter><Suspense fallback={<RouteFallback/>}><Routes><Route path="/login" element={<LoginPage/>}/><Route element={<RequireSession><Layout/></RequireSession>}><Route index element={<PassportPage/>}/><Route path="passport" element={<Navigate to="/" replace/>}/><Route path="journal" element={<LibraryPage/>}/><Route path="producers" element={<ProducersPage/>}/><Route path="producers/research-batch" element={<ResearchCampaignPage/>}/><Route path="producers/:id" element={<ProducerDetailPage/>}/><Route path="insights" element={<InsightsPage/>}/><Route path="achievements" element={<AchievementsPage/>}/><Route path="achievements/new" element={<CollectionEditorPage/>}/><Route path="achievements/:id/edit" element={<CollectionEditorPage/>}/><Route path="achievements/:id" element={<AchievementDetailPage/>}/><Route path="tastings" element={<TastingsPage/>}/><Route path="tastings/:id" element={<TastingDetailPage/>}/><Route path="upload" element={<UploadPage/>}/><Route path="group-scan" element={<GroupScanPage/>}/><Route path="batch-scan" element={<BatchScanPage/>}/><Route path="wines/new" element={<section><h1>Add wine</h1><WineForm/></section>}/><Route path="wines/:id" element={<DetailPage/>}/><Route path="wines/:id/edit" element={<Edit/>}/></Route></Routes></Suspense></BrowserRouter>}
