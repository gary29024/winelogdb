...
export const startTasting=(input:{name:string;tastingDate:string;venue?:string|null})=>
  apiFetch('/api/tastings',{method:'POST',headers:authHeaders(true),body:JSON.stringify(input)})
    .then(r=>json<{tasting:Tasting}>(r,'Could not start the tasting'));

export const getActiveTasting=()=>
  apiFetch('/api/tastings/active',{headers:authHeaders()}).then(r=>json<{tasting:Tasting|null}>(r,'Could not check for an open tasting'));

export const listTastings=()=>
  apiFetch('/api/tastings',{headers:authHeaders()}).then(r=>json<{items:TastingSummary[]}>(r,'Could not load tastings'));

export const getTasting=(id:string)=>
  apiFetch(`/api/tastings/${id}`,{headers:authHeaders()}).then(r=>json<TastingDetail>(r,'Tasting not found'));

export const updateTasting=(id:string,patch:{name?:string;venue?:string|null})=>
  apiFetch(`/api/tastings/${id}`,{method:'PUT',headers:authHeaders(true),body:JSON.stringify(patch)})
    .then(r=>json<{tasting:Tasting}>(r,'Could not update the tasting'));

export const endTasting=(id:string)=>
  apiFetch(`/api/tastings/${id}/end`,{method:'POST',headers:authHeaders()}).then(r=>json<{tasting:Tasting}>(r,'Could not end the tasting'));

export const reopenTasting=(id:string)=>
  apiFetch(`/api/tastings/${id}/reopen`,{method:'POST',headers:authHeaders()}).then(r=>json<{tasting:Tasting}>(r,'Could not reopen the tasting'));

export const attachWinesToTasting=(id:string,ids:string[])=>
  apiFetch(`/api/tastings/${id}/wines`,{method:'POST',headers:authHeaders(true),body:JSON.stringify({ids})})
...
export async function detachWineFromTasting(id:string,wineId:string){
  const response=await apiFetch(`/api/tastings/${id}/wines/${wineId}`,{method:'DELETE',headers:authHeaders()});
...
export async function deleteTasting(id:string){
  const response=await apiFetch(`/api/tastings/${id}`,{method:'DELETE',headers:authHeaders(true),body:JSON.stringify({confirmation:'DELETE_TASTING'})});
  if(response.status===401){clearSession();throw new Error('Session expired. Please sign in again.')}
...
export const uploadTastingDocuments=(id:string,files:File[])=>{
  ...
  return apiFetch(`/api/tastings/${id}/documents`,{method:'POST',headers:authHeaders(),body:form})
...
export const parseTastingSheetPage=(id:string,page:File,afterLine?:number|null,continuation?:string)=>{
  const form=new FormData();
  ...
  return apiFetch(`/api/tastings/${id}/sheet/parse`,{method:'POST',headers:{...authHeaders(),...(continuation?{'X-WineLog-Continuation':continuation}:{})},body:form})
...
export const fillTastingSheetPrices=(id:string,currency:string,prices:Array<{wineId:string;price:number}>)=>
  apiFetch(`/api/tastings/${id}/sheet/prices`,{method:'POST',headers:authHeaders(true),body:JSON.stringify({currency,prices})})
...
export const createTastingSheetWines=(id:string,body:{currency:string|null;tastingDate:string|null;venue:string|null;wines:Array<Record<string,unknown>>})=>
  apiFetch(`/api/tastings/${id}/sheet/wines`,{method:'POST',headers:authHeaders(true),body:JSON.stringify(body)})
...
export async function readTastingDocumentFile(documentId:string,name='wine-list.jpg'){
  const response=await apiFetch(`/api/tastings/documents/${documentId}`,{headers:authHeaders()});
...
export async function deleteTastingDocument(documentId:string){
  const response=await apiFetch(`/api/tastings/documents/${documentId}`,{method:'DELETE',headers:authHeaders()});
  if(response.status===401){clearSession();throw new Error('Session expired. Please sign in again.')}
...
export async function matchTastingWine(tastingId:string,wine:{producer:string;wineName:string;vintage:number|null}){
  ...
  const r=await apiFetch(`/api/tastings/${tastingId}/wine-match?${query}`,{headers:authHeaders()});
  if(!r.ok)return null;
