...
export function AchievementDetailPage(){
  ...
  return <section className="achievements-page achievement-detail-page">
    ...
    <section className="achievement-checklist-card">
      ...
      {collection.items.length?<div className="achievement-checklist">{groups.map(group=><div className="achievement-check-group" key={group.key||'all'}>{(group.section||group.subsection)&&<div className="achievement-check-section">{group.section&&<strong>{group.section}</strong>}{group.subsection&&<span>{group.subsection}</span>}</div>}{group.items.map(({item})=>{const firstWine=item.tastedWineIds[0];return <article className={`achievement-check-row achievement-check-${item.status}`} key={item.id}><span className="achievement-check-mark" aria-hidden="true">{item.status==='tasted'?'✓':item.status==='possible'?'?':'○'}</span><div className="achievement-check-copy"><strong>{item.label}</strong>{item.note&&<small>{item.note}</small>}{item.tastedVintages.length>0&&<small>Tasted vintages: {item.tastedVintages.join(', ')}</small>}</div><div className="achievement-check-status"><span>{statusCopy(item.status)}</span>{firstWine&&<Link to={`/wines/${firstWine}`} state={linkFrom({to:`/achievements/${id}`,label:definition.title})}>{item.status==='possible'?'Review':'View tasting'}</Link>}</div></article>})}</div>)}</div>:<p className="achievement-empty-checklist">No catalogue targets currently match this live rule. Edit the collection or refresh the relevant producer catalogue.</p>}
    </section>
