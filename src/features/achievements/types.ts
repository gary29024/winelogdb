...
export type ProducerSelector={type:'producer';producerNames:string[]};
export type CuveeSelector={type:'cuvee';producerNames:string[];cuveeNames:string[];appellationNames?:string[]};
export type WineVintageSelector={type:'wine_vintage';producerNames:string[];cuveeNames:string[];vintage:number;appellationNames?:string[]};
...
export type AchievementDefinition={id:string;title:string;subtitle:string;category:AchievementCategory;icon:AchievementIconKey;items:AchievementDefinitionItem[];references:AchievementReference[];series?:AchievementSeries};

export type AchievementProducerIdentity={id:string;canonicalName:string;aliases?:string[]};
export type AchievementCuveeIdentity={id:string;producerId:string;canonicalName:string;aliases?:string[];appellation?:string|null};
...
export type AchievementProgress={definition:AchievementDefinition;completed:number;possible:number;pending:number;total:number;percent:number;complete:boolean;items:AchievementItemProgress[]};