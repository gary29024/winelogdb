...
export type AchievementWine={id:string;producerId?:string|null;cuveeId?:string|null;producer:string;wineName:string;vintage?:number|null;appellation?:string|null;tastingDate?:string|null;shared?:boolean};
/** One tasted vintage and the tasting to open for it: the most recent of that vintage. */
export type AchievementVintageLink={vintage:number;wineId:string;shared?:boolean};
export type AchievementItemProgress={id:string;label:string;note?:string;status:AchievementItemStatus;tastedWineIds:string[];tastedSharedWineIds?:string[];tastedVintages:number[];tastedVintageLinks:AchievementVintageLink[];resolvedProducerId?:string;resolvedCuveeId?:string};
