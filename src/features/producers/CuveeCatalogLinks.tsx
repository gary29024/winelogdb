...
export function CuveeCatalogLinks({producer,group,onChanged}:{producer:ProducerDetail;group:TastedCuveeGroup;onChanged:()=>Promise<void>}){
  ...
  return <>
    <div className="cuvee-inline-admin">
      {error&&<small className="cuvee-inline-error" role="alert">{error}</small>}
      {!directMatch&&!link&&<button type="button" disabled={busy} onClick={openLink}>Link</button>}
      {link&&<><button type="button" disabled={busy} onClick={openLink}>Change</button><button type="button" className="secondary-danger" disabled={busy} onClick={()=>void unlink()}>Unlink</button></>}
    </div>
