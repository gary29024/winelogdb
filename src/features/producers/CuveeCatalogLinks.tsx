...
import { CATALOG_HIERARCHY_LABELS } from '../../lib/cuvees/catalogPresentation';
...
type CatalogTarget=ProducerDetail['catalogChoices'][number];
...
export function CuveeCatalogLinks({producer,group,onChanged}:{producer:ProducerDetail;group:TastedCuveeGroup;onChanged:()=>Promise<void>}){
  ...
  const choices=useMemo<CatalogTarget[]>(()=>producer.catalogChoices??producer.catalogCuvees.map(target=>({key:target.id,id:target.id,canonicalName:target.canonicalName,appellation:target.appellation,wineStyle:target.wineStyle,classification:null,hierarchy:'Other / unclassified',issue:null})),[producer.catalogChoices,producer.catalogCuvees]);
