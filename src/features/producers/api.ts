...
export const getProducerResearchStatus=(id:string,requestId:string)=>fetch(`/api/producers/${id}/research-status?requestId=${encodeURIComponent(requestId)}`,{headers:authHeaders()}).then(r=>json<ProducerResearchRun>(r,'Could not load producer research status'));
export const researchProducer=(id:string,requestId:string)=>fetch(`/api/producers/${id}/research`,{method:'POST',headers:authHeaders(true),body:JSON.stringify({confirmation:'RUN_PRODUCER_RESEARCH',requestId})}).then(r=>json<ProducerEntity&{researchRequestId?:string}>(r,'Producer research failed'));
