...
export const listProducers=()=>apiFetch('/api/producers',{headers:authHeaders()}).then(r=>json<{items:ProducerSummary[]}>(r,'Could not load producers'));
export const resolveProducer=(name:string)=>apiFetch(`/api/producers/resolve?name=${encodeURIComponent(name)}`,{headers:authHeaders()}).then(r=>json<ProducerResolution>(r,'Could not resolve producer'));
export const getProducer=(id:string)=>apiFetch(`/api/producers/${id}`,{headers:authHeaders()}).then(r=>json<ProducerDetail>(r,'Producer not found')).then(detail=>{
...
export const setPrimaryProducerName=(id:string,name:string)=>apiFetch(`/api/producers/${id}/primary-name`,{method:'POST',headers:authHeaders(true),body:JSON.stringify({name})}).then(r=>json<{id:string;canonicalName:string}>(r,'Could not change primary name'));
...
export const deleteProducer=(id:string)=>apiFetch(`/api/producers/${id}`,{method:'DELETE',headers:authHeaders(true),body:JSON.stringify({confirmation:'DELETE_PRODUCER'})}).then(r=>json<{id:string;canonicalName:string;deleted:true}>(r,'Could not delete this producer'));
export const unlinkProducer=(destinationId:string,mergeId:string)=>apiFetch(`/api/producers/${destinationId}/unlink`,{method:'POST',headers:authHeaders(true),body:JSON.stringify({confirmation:'UNLINK_PRODUCER',mergeId})}).then(r=>json<{destinationId:string;restoredProducerId:string;canonicalName:string;unlinkedName:string}>(r,'Could not unlink producer'));
export const createSupplementaryContact=(producerId:string,input:ManualProducerContactInput)=>apiFetch(`/api/producers/${producerId}/manual-contacts`,{method:'POST',headers:authHeaders(true),body:JSON.stringify(input)}).then(r=>json<ManualProducerContact>(r,'Could not add supplementary contact'));
export const updateSupplementaryContact=(producerId:string,contactId:string,input:ManualProducerContactInput)=>apiFetch(`/api/producers/${producerId}/manual-contacts/${contactId}`,{method:'PUT',headers:authHeaders(true),body:JSON.stringify(input)}).then(r=>json<ManualProducerContact>(r,'Could not update supplementary contact'));
export const deleteSupplementaryContact=(producerId:string,contactId:string)=>apiFetch(`/api/producers/${producerId}/manual-contacts/${contactId}`,{method:'DELETE',headers:authHeaders(true),body:JSON.stringify({confirmation:'DELETE_MANUAL_CONTACT'})}).then(r=>json<{id:string;deleted:true}>(r,'Could not delete supplementary contact'));
export const linkTastedCuveeToCatalog=(producerId:string,sourceCuveeId:string,catalogCuveeId:string)=>apiFetch(`/api/producers/${producerId}/cuvee-links`,{method:'POST',headers:authHeaders(true),body:JSON.stringify({confirmation:'LINK_CUVEE_TO_CATALOG',sourceCuveeId,catalogCuveeId})}).then(r=>json<{id:string;sourceCuveeId:string;catalogCuveeId:string;existing:boolean}>(r,'Could not link tasted cuvée to catalog'));
export const changeTastedCuveeCatalogLink=(producerId:string,linkId:string,catalogCuveeId:string)=>apiFetch(`/api/producers/${producerId}/cuvee-links/${linkId}`,{method:'PUT',headers:authHeaders(true),body:JSON.stringify({confirmation:'CHANGE_CUVEE_CATALOG_LINK',catalogCuveeId})}).then(r=>json<{id:string;sourceCuveeId:string;catalogCuveeId:string;changed:boolean}>(r,'Could not change catalog link'));
export const unlinkTastedCuveeFromCatalog=(producerId:string,linkId:string)=>apiFetch(`/api/producers/${producerId}/cuvee-links/${linkId}/unlink`,{method:'POST',headers:authHeaders(true),body:JSON.stringify({confirmation:'UNLINK_CUVEE_FROM_CATALOG'})}).then(r=>json<{id:string;sourceCuveeId:string;catalogCuveeId:string;unlinked:boolean}>(r,'Could not unlink tasted cuvée from catalog'));
...
export const saveProducerCatalogDecision=(producerId:string,input:{decision:CatalogDecisionKind;sourceKey:string;sourceName:string;targetKey?:string|null;targetName?:string|null})=>
  apiFetch(`/api/producers/${producerId}/catalog-decisions`,{method:'POST',headers:authHeaders(true),body:JSON.stringify({confirmation:'CORRECT_PRODUCER_CATALOG',...input})})
...
export const undoProducerCatalogDecision=(producerId:string,decisionId:string)=>
  apiFetch(`/api/producers/${producerId}/catalog-decisions/${decisionId}/undo`,{method:'POST',headers:authHeaders(true),body:JSON.stringify({confirmation:'UNDO_PRODUCER_CATALOG_CORRECTION'})})
...
export const getResearchCampaignPlan=(limit?:number)=>
  apiFetch(`/api/producers/research-batch/plan${limit?`?limit=${limit}`:''}`,{headers:authHeaders()})
    .then(r=>json<ResearchCampaignPlan>(r,'Could not work out what a batch run would involve'));
export const getResearchCampaign=()=>
  apiFetch('/api/producers/research-batch',{headers:authHeaders()}).then(r=>json<{campaign:ResearchCampaign|null}>(r,'Could not load the batch run')).then(r=>r.campaign);
export const startResearchCampaign=(limit:number)=>
  apiFetch('/api/producers/research-batch',{method:'POST',headers:authHeaders(true),body:JSON.stringify({confirmation:'RUN_PRODUCER_RESEARCH_BATCH',limit})})
...
export const listResearchCampaigns=(limit=10)=>
  apiFetch(`/api/producers/research-batch/history?limit=${limit}`,{headers:authHeaders()})
...
export const getResearchCampaignById=(id:string)=>
  apiFetch(`/api/producers/research-batch/${id}`,{headers:authHeaders()})
...
export const cancelResearchCampaign=(id:string)=>
  apiFetch(`/api/producers/research-batch/${id}/cancel`,{method:'POST',headers:authHeaders(true),body:'{}'})
...
export const dismissResearchCampaign=(id:string)=>
  apiFetch(`/api/producers/research-batch/${id}/dismiss`,{method:'POST',headers:authHeaders(true),body:'{}'})
