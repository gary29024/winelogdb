...
export function ProducersPage(){
 ...
 return <section className="producer-page"><div className="hero compact"><p className="eyebrow">PRODUCERS</p><h1>Your producer library.</h1><p>Browse domaines by where they are physically based: country, broad wine region, then commune — not by the appellations represented in their wines.</p></div>
  <div className="producer-search"><div className="producer-search-field"><span aria-hidden="true"><AppIcon kind="search"/></span><input type="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search producers…" aria-label="Search producers"/>{hasQuery&&<button type="button" onClick={()=>setQuery('')} aria-label="Clear producer search">Clear</button>}</div>{!loading&&!error&&<small>{hasQuery?`${filteredItems.length} of ${items.length} producers`:`${items.length} producer${items.length===1?'':'s'}`}</small>}</div>
  {loading?<p>Loading producers…</p>:error?<p role="alert">{error}</p>:groups.length?<>
  {collapsible&&<div className="producer-range-head producer-countries-head"><span>{groups.length} countries · {filteredItems.length} producer{filteredItems.length===1?'':'s'}</span><button type="button" className="range-toggle-all" onClick={toggleAllCountries}>{allOpen?'Collapse all':'Expand all'}</button></div>}
