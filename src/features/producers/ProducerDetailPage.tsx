...
export function ProducerDetailPage(){
 ...
 async function runResearch(){
  if(!confirm('Research this producer’s home location and current wine range with Gemini + Google Search? WineLog will also try to save a header image from the verified official website when available.'))return;
 ...
 return <article className="producer-detail"><Link className="back-pill" to="/producers">← Producers</Link>
  ...
  <section className="detail-section"><div className="producer-section-title"><div><p className="section-label">PRODUCER RESEARCH</p><h2>Profile & range</h2></div><button type="button" disabled={researching} onClick={runResearch}>{researching?'Researching…':producer.researchedAt?'Refresh producer research':'Research producer'}</button></div>
   ...
   {producer.profile?<p className="producer-profile">{producer.profile}</p>:<p>Research this producer to establish its physical base, official website, header image and a sourced current/recent wine range. Producer-level research is stored and reused.</p>}
