...
export function ProducerDetailPage(){
 ...
 const tastedGroups=useMemo<TastedCuveeGroup[]>(()=>{
  ...
  for(const wine of producer?.tastedWines??[]){const style=cuveeStyleFamily(wine.wineStyle)||'unknown',key=`${wine.cuveeId??normalizeProducerAlias(wine.wineName)}::${style}`,list=map.get(key)??[];list.push(wine);map.set(key,list)}
  return [...map.values()].map(wines=>{const first=wines[0],grapes=[...new Set(wines.flatMap(wine=>wine.grapes??[]).map(grape=>grape.trim()).filter(Boolean))];return {cuveeId:first?.cuveeId??null,name:first?.wineName??'',appellation:first?.appellation??null,wineStyle:first?.wineStyle??null,grapes,wines:[...wines].sort((a,b)=>(b.vintage??-1)-(a.vintage??-1))}});
 },[producer]);
 ...
 const location=[producer.homeLocality,producer.homeRegion,producer.homeCountry].filter(Boolean).join(', '),primaryKey=normalizeProducerAlias(producer.canonicalName),hasContact=Boolean(producer.officialWebsiteUrl||producer.instagramUrl||producer.contactEmail||producer.contactPhone),sourceWebsiteCount=new Set(producer.sources.map(source=>sourceHost(source.url)).filter(Boolean)).size;
 return <article className="producer-detail"><Link className="back-pill" to="/producers">← Producers</Link>
  ...
  <section className="detail-section"><div className="producer-section-title"><div><p className="section-label">PRODUCER RESEARCH</p><h2>Profile & range</h2></div><button type="button" disabled={researching} onClick={runResearch}>{researching?'Research running…':producer.researchedAt?'Refresh producer research':'Research producer'}</button></div>
   ...
   {catalogGroups.map(group=><div className="producer-catalog-group" key={group.category}><h3>{group.label}<span>{group.wines.length}</span></h3><div className="producer-catalog">{group.wines.map((wine,index)=>{const displayName=displayCatalogName(wine.name,producer),catalogIdentity=catalogCuveeFor(wine,producer),meta=catalogMeta(wine,group.category),note=catalogNote(wine);return <div className="catalog-row" key={`${wine.name}-${index}`}><div style={{minWidth:0}}><strong>{displayName}</strong>{meta.length>0&&<span className="catalog-meta">{meta.join(' · ')}</span>}{note.short&&<small className="catalog-notes" style={{overflowWrap:'anywhere',wordBreak:'break-word'}} title={note.full}>{note.short}</small>}</div>{Boolean(catalogIdentity?.tastedCount)&&<span className="tasted-badge">Tasted{catalogIdentity&&catalogIdentity.tastedCount>1?` · ${catalogIdentity.tastedCount}`:''}</span>}</div>})}</div></div>)}
  ...
  <section className="detail-section"><p className="section-label">YOUR TASTINGS</p><h2>{tastedGroups.length} cuvée{tastedGroups.length===1?'':'s'} · {producer.tastedWines.length} tasting{producer.tastedWines.length===1?'':'s'}</h2>{tastedGroups.length?<div className="producer-tasted-groups">{tastedGroups.map(group=>{const identityMeta=[group.wineStyle,group.grapes.length?group.grapes.join(' / '):null].filter(Boolean).join(' · ');return <div className="tasted-cuvee-group" key={`${group.cuveeId??normalizeProducerAlias(group.name)}-${cuveeStyleFamily(group.wineStyle)||'unknown'}`}><div className="tasted-cuvee-title"><div><strong>{group.name}</strong>{identityMeta&&<small>{identityMeta}</small>}</div></div><div className="producer-tasted">{group.wines.map((w,index)=><div className="tasted-row tasted-vintage-row" key={w.id}><Link to={`/wines/${w.id}`} className="tasted-row-link"><div className="tasted-thumb">{w.imageId?<WineImage imageId={w.imageId} alt={`${w.wineName} ${w.vintage??'NV'} bottle`} className="tasted-thumb-image"/>:<span className="tasted-thumb-fallback">W</span>}</div><div className="tasted-copy"><strong>{w.vintage??'NV'}</strong><span>{[w.appellation,w.region].filter(Boolean).join(' · ')}</span></div></Link><div className="tasted-meta">{w.rating!=null&&<strong>{w.rating}</strong>}{w.tastingDate&&<span>{w.tastingDate}</span>}{index===0&&<CuveeCatalogLinks producer={producer} group={group} onChanged={reload}/>}</div></div>)}</div></div>})}</div>:<p>No tasting records linked to this producer yet.</p>}
  </section>
