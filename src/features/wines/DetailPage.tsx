 ...
 return <article className="detail wine-detail"><Link className="back-pill" to={back.to}>← {back.label}</Link>
  ...
  <section className="detail-section deep-search-panel">
   ...
   {deep?<>
    ...
    <small>Latest research model: {deep.model} · {new Date(deep.researchedAt).toLocaleDateString()} · reusable research is stored permanently{isResearchStale(deep.researchedAt)&&<> · ⚠ may be outdated</>}</small>
