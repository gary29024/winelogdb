...
export function DetailPage(){
 ...
 async function checkDeepSearch(){
  if(deepChecking)return;setDeepChecking(true);setDeepError('');
  try{const run=await getWineDeepSearchStatus(id,deepRun?.requestId);if(run){watchDeepSearch(run);if(run.status==='complete')await reloadWine()}else setDeepError('Could not find the previous request. Please check again.')}
