...
import { addWineImages,applyWineReferenceSuggestion,cancelWineDeepSearch,checkWineDeepSearchStatus,deleteWine,deleteWineImage,getWine,getWineDeepSearchStatus,setWineFavorite,startWineDeepSearch,type WineDetail,type WineResearchRun } from './api';
...
const deepSearchHeld=(run:WineResearchRun|null)=>run?.retryBlocked??/Provider completion needs reconciliation/.test(run?.message??'');
const memberResearchFailure=(run:WineResearchRun|null)=>`${run?.outcome==='uncertain'?'WineLog could not confirm the research outcome.':'Deep Search did not complete.'} Any saved research is kept.${run?.requestId?` · Support ID ${run.requestId}`:''}`;
...
export function DetailPage(){
 ...
 function watchDeepSearch(run:WineResearchRun){
  stopDeepTimers();setDeepRun(run);if(run.waitingForFriend&&run.creditOperationId){setFriendOperation(run.creditOperationId);setDeepState('idle');return}setFriendOperation('');setDeepState(run.status==='running'?'running':run.status==='failed'?'error':'idle');if(run.status!=='running')return;
  const poll=async()=>{const next=await getWineDeepSearchStatus(id,run.requestId).catch(()=>null);if(!next)return;setDeepRun(next);if(next.status==='running')return;stopDeepTimers();if(next.status==='complete'){await reloadWine().catch(()=>undefined);setDeepState('idle');setDeepError('');setDeepNotice(`Deep Search completed${next.durationMs!=null?` in ${(next.durationMs/1000).toFixed(1)}s`:''}.`)}else{setDeepError(technicalView?`${next.message||'The background Deep Search failed.'} · Request ${next.requestId}`:memberResearchFailure(next));setDeepState('error')}};
  pollRef.current=startBackoffPoll(poll);void poll();
 }
 useEffect(()=>{let active=true;Promise.all([getWine(id),getWineDeepSearchStatus(id).catch(()=>null)]).then(([next,run])=>{if(!active)return;setWine(next);if(run&&(run.status==='running'||run.status==='failed'))watchDeepSearch(run)}).catch(()=>undefined);return()=>{active=false;stopDeepTimers()}// eslint-disable-next-line react-hooks/exhaustive-deps
 },[id]);
 async function checkDeepSearch(){
  if(deepChecking||deepCancelling)return;setDeepChecking(true);setDeepError('');setDeepNotice('');
  try{
   const run=await checkWineDeepSearchStatus(id,deepRun?.requestId);
   if(!run){setDeepError('Could not find the previous request. Please check again.');return}
   watchDeepSearch(run);
   const checked=`Checked at ${new Date().toLocaleTimeString()}. `;
   if(run.status==='complete'){await reloadWine();setDeepNotice(checked+'Deep Search completed.')}
   else if(deepSearchHeld(run))setDeepNotice(checked+'Still waiting for a saved result. No new research was started.');
   else if(run.status==='running')setDeepNotice(checked+(run.stage==='saving'?'A saved result was found and is being processed.':'Research is still running.'));
   else{await reloadWine();setDeepNotice(checked+'The previous request has ended. You can retry Deep Search.')}
  }catch(e){setDeepError((e as Error).message)}finally{setDeepChecking(false)}
 }
 ...
 async function cancelDeepSearch(){
  if(!deepRun||deepRun.status!=='running'&&!deepHeld||deepCancelling||deepChecking)return;
  if(!confirm(deepHeld?'Stop waiting for this request? Any saved research will be kept. The provider may already have processed the request; stopping here cannot undo that. You can start a separate retry afterwards.':'Cancel this Deep Search? Any producer, terroir, vintage or wine research already saved will be kept.'))return;
  setDeepCancelling(true);setDeepError('');setDeepNotice('');
  try{
   const result=await cancelWineDeepSearch(id,deepRun.requestId,deepHeld);
   stopDeepTimers();await reloadWine().catch(()=>undefined);
   // Re-read after cancellation: an active provider hold may still need review,
   // or a saved result may have won the race with the cancellation request.
   const run=await getWineDeepSearchStatus(id,deepRun.requestId);
   if(run)watchDeepSearch(run);else{setDeepRun(null);setDeepState('idle')}
   setDeepNotice(result.alreadyTerminal?'The previous request has already ended.':deepHeld?'Stopped waiting. Any saved research was kept. You can retry Deep Search.':'Deep Search cancelled. Any research already saved was kept.');
  }catch(e){setDeepError((e as Error).message)}finally{setDeepCancelling(false)}
 }
 /**
