...
export function DetailPage(){
 const {id=''}=useParams(),nav=useNavigate(),{state}=useLocation(),[wine,setWine]=useState<WineDetail>(),[favoriteBusy,setFavoriteBusy]=useState(false),[deepState,setDeepState]=useState<DeepState>('idle'),[deepScope,setDeepScope]=useState<'vintage'|'all'>('vintage'),[deepError,setDeepError]=useState(''),[deepRun,setDeepRun]=useState<WineResearchRun|null>(null),[deepElapsed,setDeepElapsed]=useState(0),[deepNotice,setDeepNotice]=useState(''),[deepCancelling,setDeepCancelling]=useState(false),[selectedImage,setSelectedImage]=useState<string>(),[selectedGroupSource,setSelectedGroupSource]=useState<string>(),[openDeepFields,setOpenDeepFields]=useState<Set<DeepField>>(readOpenDeepFields);
 ...
 return <article className="detail wine-detail"><Link className="back-pill" to={back.to}>← {back.label}</Link>
  ...
  <section className="detail-section deep-search-panel">
   ...
   {deepNotice&&<p className="producer-notice" role="status">{deepNotice}</p>}{deepState==='idle'&&<div className="deep-actions">
     <button type="button" className="primary" onClick={()=>{setDeepScope('vintage');setDeepState('confirm-usage')}}>{deep?'Refresh vintage research':'Deep Search'}</button>
     {/* Cached research is the point - it is what makes a refresh cheap - but a
         wrong or thin answer stays until it is cleared, and nothing could clear
         it. This drops every stored scope for this wine and starts over. */}
     {deep&&<button type="button" className="deep-reset" onClick={()=>{setDeepScope('all');setDeepState('confirm-usage')}}>Reset and search fresh</button>}
    </div>}{deepState==='confirm-usage'&&<div className="deep-confirm"><p>{deepScope==='all'?'This discards everything WineLog has stored for this wine - producer practices, terroir, vintage context and the wine-vintage research - and researches all of it again from scratch. It costs more than a refresh, and the current text is lost.':deep?'This refresh keeps cached producer-wide practices and stable terroir research, and re-runs only vintage-sensitive research for this wine.':'WineLog checks permanent caches first and queues Gemini 3.7 Flash with Google Search only for missing research scopes.'} The background job continues even if you close WineLog. API usage may be incurred. Continue?</p><button type="button" className="primary" onClick={runDeepSearch}>{deepScope==='all'?'Clear and research again':deep?'Queue vintage refresh':'Queue Deep Search'}</button><button type="button" className="secondary-danger" onClick={()=>setDeepState('idle')}>Cancel</button></div>}{deepState==='running'&&<div className="deep-running" role="status"><span className="deep-spinner" aria-hidden="true"/><div><strong>{deepRun?deepStage[deepRun.stage]:'Queueing Deep Search…'}</strong><p>{deepRun?.message||'Preparing the background job.'}</p><small>{deepElapsed}s{deepRun?` · Request ${deepRun.requestId}`:''}</small><p>You can leave this page or close WineLog. The Queue continues independently and the saved result will appear when you return.</p><button type="button" className="secondary-danger" disabled={!deepRun||deepCancelling} onClick={cancelDeepSearch}>{deepCancelling?'Cancelling…':'Cancel Deep Search'}</button></div></div>}{deepState==='error'&&<div className="deep-error" role="alert"><strong>Deep Search did not complete.</strong><p>{deepError||deepRun?.message||'The background research job failed before a result was saved.'}</p><button type="button" onClick={runDeepSearch}>Retry Deep Search</button><button type="button" className="secondary-danger" onClick={()=>setDeepState('idle')}>Close</button></div>}
  </section>
