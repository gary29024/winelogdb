...
function WineCard({wine:w,view,selecting,selected,onToggle,onFavorite,favoriteBusy,backTo}:{wine:JournalWine;view:ViewMode;selecting:boolean;selected:boolean;onToggle:()=>void;onFavorite:(next:boolean)=>void;favoriteBusy:boolean;backTo:BackTarget}){
  const image=w.shared&&w.imageUrl?<img src={w.imageUrl} alt={`${w.producer} ${w.wineName} front label`} className="journal-wine-thumb" loading="lazy" decoding="async"/>:w.imageIds[0]?<WineImage imageId={w.imageIds[0]} alt={`${w.producer} ${w.wineName} front label`} className="journal-wine-thumb"/>:<div className={`bottle bottle-${pourFamily(w.wineStyle)}`}>{w.wineStyle?.slice(0,1).toUpperCase()||'W'}</div>;
  const selectable=selecting&&!w.shared;
  const className=`wine-card journal-card ${view==='grid'?'journal-grid-card':''}${w.shared?' shared':''}${selectable?' journal-selectable-card':''}${selected?' selected':''}`;
...
export function LibraryPage(){
  ...
  const [view,setViewState]=useState<ViewMode>(initialView),[refreshSeq,setRefreshSeq]=useState(0);
  // Search text lives in JournalSearchInput so each keystroke does not traverse
  // the wine grid. This counter is only for an explicit Reset when the draft has
  ...
  const batchDialog=useRef<HTMLDivElement>(null),batchTrigger=useRef<HTMLButtonElement>(null);
  useModalFocus(batchOpen,batchDialog,()=>setBatchOpen(false),batchBusy,batchTrigger);
  const [tagOpen,setTagOpen]=useState(false),[tagBusy,setTagBusy]=useState(false),[tagError,setTagError]=useState(''),[tagFriends,setTagFriends]=useState<FriendTag[]>([]),[tagSelected,setTagSelected]=useState<string[]>([]);
  ...
  function stopSelecting(){setSelecting(false);setSelectedIds(new Set());setBatchOpen(false);setTagOpen(false);setBatchError('');setTagError('')}
  function toggleSelection(id:string){
    if(data.find(wine=>wine.id===id)?.shared)return;
  ...
  function selectAllOnPage(){
    const own=data.filter(w=>!w.shared),ids=own.slice(0,MAX_BATCH_SELECTION).map(w=>w.id);setSelectedIds(new Set(ids));
   ...
   */
  async function openFriendTagging(){
    if(!selectedWines.length)return;
    setTagOpen(true);setTagSelected([]);setTagError('');
    try{setTagFriends((await listFriendTags()).items)}catch(e){setTagError((e as Error).message)}
  }
  async function saveFriendTags(){
    if(!selectedWines.length||!tagSelected.length){setTagError('Choose at least one friend.');return}
    setTagBusy(true);setTagError('');setBatchNotice('');
    try{
      await setBulkWineFriendTags(selectedWines.map(wine=>wine.id),tagSelected);
      setTagOpen(false);
  ...
  useEffect(()=>{
    ...
    // Keep the previous page painted while the next query is in flight. On a
    // phone this avoids tearing down and rebuilding dozens of image cards just
    // to show a loading line, and makes a settled search feel continuous.
    setLoading(true);setError('');setSelectedIds(new Set());setSelecting(Boolean(attachTo));setBatchOpen(false);
    listWines(params,{limit:PAGE_SIZE,offset:currentOffset,signal:controller.signal})
      .then(result=>{
        if(controller.signal.aborted)return;
        // A saved page can disappear after deletion or unsharing. Keep the
        // filters and return to the last remaining page before showing results.
        if(currentOffset>0&&currentOffset>=result.total){
          const lastOffset=Math.max(0,(Math.ceil(result.total/PAGE_SIZE)-1)*PAGE_SIZE);
          redirecting=true;
          setParams(previous=>{const next=new URLSearchParams(previous);if(lastOffset)next.set('offset',String(lastOffset));else next.delete('offset');return next},{replace:true});
          return;
        }
        setData(result.items);setNextOffset(result.nextOffset);setTotal(result.total);
      })
      .catch(e=>{if(e?.name!=='AbortError')setError(e.message)})
      .finally(()=>{if(!controller.signal.aborted&&!redirecting)setLoading(false)});
    return()=>controller.abort();
  },[queryKey,refreshSeq,restoring,attachTo,currentOffset,params,setParams]);
  ...
  const ordered=useMemo(()=>chronological?[...data].sort((a,b)=>sort==='oldest'?journalDate(a).localeCompare(journalDate(b)):journalDate(b).localeCompare(journalDate(a))):data,[chronological,data,sort]);
  const groups=useMemo(()=>chronological?ordered.reduce<Array<{key:string;items:JournalWine[]}>>((acc,wine)=>{const key=monthKey(wine),last=acc[acc.length-1];if(last?.key===key)last.items.push(wine);else acc.push({key,items:[wine]});return acc},[]):[],[chronological,ordered]);
  ...
  return <section className="journal-page">
    {favoriteOnly
      ?<PageIntro title="Favorites" description="The bottles you loved most, all in one place."/>
