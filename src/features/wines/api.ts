...
export type WineResearchRun={requestId:string;wineId:string;status:'running'|'complete'|'failed';stage:WineResearchStage;refresh:'none'|'vintage'|'all';attempt:number;message:string|null;startedAt:string;updatedAt:string;completedAt:string|null;durationMs:number|null;retryBlocked?:boolean;recoveryDeadline?:string;outcome?:'uncertain'|'interrupted';waitingForFriend?:boolean;creditOperationId?:string};
...
export async function applyWineReferenceSuggestion(id:string,field:WineDetail['referenceSuggestions'][number]['field'],action:'apply'|'keep'='apply'){const r=await apiFetch(`/api/wines/${id}/reference-suggestion`,{method:'PUT',headers:authHeaders(true),body:JSON.stringify({field,action})});await requireOk(r,'Could not apply LWIN suggestion');summariesChanged();return r.json() as Promise<{ok:true;referenceSuggestions:WineDetail['referenceSuggestions']}>}
...
export async function addWineImages(id:string,photos:WinePhoto[]){
  ...
  const r=await apiFetch(`/api/wines/${id}/images`,{method:'POST',headers:authHeaders(),body:fd});
...
export async function deleteWineImage(wineId:string,imageId:string){
  const r=await apiFetch(`/api/wines/${wineId}/images/${imageId}`,{method:'DELETE',headers:authHeaders()});
...
async function readWineDeepSearchStatus(id:string,requestId:string|undefined,method:'GET'|'POST'){const suffix=requestId?`?requestId=${encodeURIComponent(requestId)}`:'';const r=await apiFetch(`/api/wines/${id}/deep-search-status${suffix}`,{method,headers:authHeaders()});if(r.status===404)return null;await requireOk(r,'Could not load Deep Search status');return r.json() as Promise<WineResearchRun>}
export const getWineDeepSearchStatus=(id:string,requestId?:string)=>readWineDeepSearchStatus(id,requestId,'GET');
export const checkWineDeepSearchStatus=(id:string,requestId?:string)=>readWineDeepSearchStatus(id,requestId,'POST');
export async function cancelWineDeepSearch(id:string,requestId:string,stopWaiting=false){const r=await apiFetch(`/api/wines/${id}/deep-search-cancel`,{method:'POST',headers:authHeaders(true),body:JSON.stringify({confirmation:stopWaiting?'STOP_WAITING_DEEP_SEARCH':'CANCEL_DEEP_SEARCH',requestId})});await requireOk(r,'Could not cancel Deep Search');return r.json() as Promise<WineResearchCancelResult>}
