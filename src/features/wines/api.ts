...
export async function applyWineReferenceSuggestion(id:string,field:WineDetail['referenceSuggestions'][number]['field']){const r=await apiFetch(`/api/wines/${id}/reference-suggestion`,{method:'PUT',headers:authHeaders(true),body:JSON.stringify({field})});await requireOk(r,'Could not apply LWIN suggestion');summariesChanged();return r.json() as Promise<{ok:true;referenceSuggestions:WineDetail['referenceSuggestions']}>}
