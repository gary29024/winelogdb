...
export async function saveWineSparklingDetails(id:string,details:SparklingDetails|null){const r=await fetch(`/api/wines/${id}/sparkling-details`,{method:'PUT',headers:authHeaders(true),body:JSON.stringify({details})});await requireOk(r,'Could not save sparkling details');return r.json() as Promise<{ok:true}>}
