...
export async function getWineSparklingDetails(id:string){const r=await fetch(`/api/wines/${id}/sparkling-details`,{headers:authHeaders()});await requireOk(r,'Could not load sparkling details');return r.json() as Promise<{details:SparklingDetails|null}>}
export async function saveWineSparklingDetails(id:string,details:SparklingDetails|null){const r=await fetch(`/api/wines/${id}/sparkling-details`,{method:'PUT',headers:authHeaders(true),body:JSON.stringify({details})});await requireOk(r,'Could not save sparkling details');summariesChanged();return r.json() as Promise<{ok:true}>}
