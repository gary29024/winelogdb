 ...
 return <section className="lwin-review-page">
  ...
  <div className="lwin-review-list">{page?.items.map(item=><article className={`lwin-review-card${selected===item.id?' is-open':''}`} key={item.id}>
   ...
   {selected===item.id&&(!wine?<p>Loading review…</p>:<div className="lwin-review-body">
    ...
    <LwinLinkEditor key={wine.id} wine={wine} disabled={busy} onLink={preview=>act(()=>apiJson(`/api/wines/${wine.id}/reference-review`,'POST',{action:'link',lwin7:preview.requestedLwin7,previewToken:preview.previewToken}))} onReject={()=>act(()=>apiJson(`/api/wines/${wine.id}/reference-review`,'POST',{action:'reject',lwin7:wine.lwin7??null,updatedAt:wine.updatedAt}))}/>
