  ...
  return <details className="sparkling-details-editor" open={open} onToggle={e=>setOpen(e.currentTarget.open)}>
    ...
    <div className="sparkling-details-body">
      ...
      <div className="sparkling-spec-grid">
        ...
        <label>Dosage style<input value={details.dosageCategory??''} placeholder="Extra Brut" onChange={e=>set('dosageCategory',e.target.value||null)}/></label>
        <label>Disgorged<input value={details.disgorgement??''} placeholder="03/2024" onChange={e=>set('disgorgement',e.target.value||null)}/></label>
        <label>Tirage / bottled<input value={details.tirage??''} placeholder="07/2019" onChange={e=>set('tirage',e.target.value||null)}/></label>
        ...
        <label>Lot / release<input value={details.lotCode??''} placeholder="L23 / DT0324" onChange={e=>set('lotCode',e.target.value||null)}/></label>
      </div>
      <div className="sparkling-text-grid">
        <label>Assemblage<textarea rows={2} value={details.assemblage??''} placeholder="2019 base + 30% reserve wines…" onChange={e=>set('assemblage',e.target.value||null)}/></label>
        <label>Reserve wine detail<textarea rows={2} value={details.reserveWineDetail??''} placeholder="Perpetual reserve, 2015–2018…" onChange={e=>set('reserveWineDetail',e.target.value||null)}/></label>
        <label>Malolactic<input value={details.malolactic??''} placeholder="Blocked / partial / full" onChange={e=>set('malolactic',e.target.value||null)}/></label>
        <label>Fermentation / élevage<input value={details.fermentationElevage??''} placeholder="70% steel, 30% oak…" onChange={e=>set('fermentationElevage',e.target.value||null)}/></label>
      </div>
