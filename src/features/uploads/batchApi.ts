...
export async function stageBatchWine(sessionId:string,position:number,photos:StagePhoto[],signal?:AbortSignal){const fd=new FormData();for(const photo of photos){fd.append('originals',photo.original);fd.append('recognitionImages',photo.recognition)}fd.append('position',String(position));fd.append('metadata',JSON.stringify(photos.map(x=>x.metadata)));fd.append('dimensions',JSON.stringify(photos.map(x=>({width:x.width,height:x.height}))));return read<{id:string;position:number;photoCount:number;preparedBytes:number}>(await fetch(`/api/batch-recognition/sessions/${sessionId}/items`,{method:'POST',headers:authHeaders(),body:fd,signal}),'Could not stage wine')}
