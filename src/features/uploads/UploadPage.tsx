...
export function UploadPage(){
  ...
  return <section className="scan-page">
    ...
    {review&&<div className="review"><p className="eyebrow">REVIEW</p><h2>Combined identification</h2><p>Gemini interpreted all selected labels together. Tap any label thumbnail above to inspect the original at a larger size, then correct anything before saving. Exact photo coordinates are retained from EXIF; when GPS exists, Gemini may also suggest an approximate human-readable place that you can verify or edit. The original photos are written to R2 only after the wine is successfully logged.</p><WineForm photos={photos} initial={{producer:review.producer??'',wineName:review.wineName??'',vintage:review.vintage,country:review.country,region:review.region,appellation:review.appellation,recognizedRegion:review.recognizedRegion,recognizedAppellation:review.recognizedAppellation,grapes:review.grapes,grapeBlend:review.grapeBlend,wineStyle:review.style,alcoholPercentage:review.alcoholPercentage,tastingDate:review.tastingDate,locationName:review.locationName,latitude:review.latitude,longitude:review.longitude,tags:suggestedTags(review),recognitionConfidence:review.confidence,recognitionStatus:'review'}}/></div>}
