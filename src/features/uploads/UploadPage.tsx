...
function mergeRecognition(results:RecognitionResult[]):RecognitionResult{
  ...
  merged.grapes=[...new Set(ranked.flatMap(x=>x.grapes))];
  const blend=new Map<string,{grape:string;percentage?:number|null}>();
  for(const item of ranked.flatMap(x=>x.grapeBlend)){if(!blend.has(item.grape.toLowerCase()))blend.set(item.grape.toLowerCase(),item)}
  merged.grapeBlend=[...blend.values()];
  if(!merged.grapes.length&&merged.grapeBlend.length)merged.grapes=merged.grapeBlend.map(x=>x.grape);
