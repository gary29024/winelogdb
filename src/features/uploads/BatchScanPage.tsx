...
export function BatchScanPage(){
  ...
  const poll=useRef<Poller|undefined>(undefined),uploadAbort=useRef<AbortController|null>(null),cancelledSessions=useRef(new Set<string>()),recoveringSessions=useRef(new Set<string>());
  ...
  async function recoverWaitingRecognition(next:BatchRecognitionSession){
    const waiting=next.items.filter(item=>item.status==='submitted').length;
    if(!waiting||!['ready','partial','failed'].includes(next.status)||recoveringSessions.current.has(next.id))return next;
    recoveringSessions.current.add(next.id);
    try{
      await submitBatchSession(next.id);
      setNotice(`Recovered ${waiting} wine${waiting===1?'':'s'} that had been submitted without an active recognition job. Recognition has been queued again automatically.`);
      ...
      return latest??next;
    }finally{recoveringSessions.current.delete(next.id)}
  }
  ...
  if(session)return <section className="batch-scan-page"><div className="hero compact"><p className="eyebrow">BATCH SCAN</p><h1>{session.totalItems} wines in this batch.</h1><p>{session.status==='queued'||session.status==='running'?'Gemini is processing these independent wine requests in the background. You can close WineLog safely.':waitingItems.length?`${waitingItems.length} waiting for recognition · ${readyItems.length} ready for review · ${session.confirmedItems} confirmed.`:`${readyItems.length} ready for review · ${session.confirmedItems} confirmed.`}</p></div>
    ...
    {waitingItems.length>0&&['ready','partial','failed'].includes(session.status)&&<div className="batch-upload-panel"><div className="batch-upload-progress"><strong>RECOGNITION INTERRUPTED</strong><span>{waitingItems.length} waiting</span></div><small>These photos reached WineLog but no active Gemini job remains for them. WineLog will retry them without re-uploading the photos.</small><div className="batch-upload-actions"><button type="button" onClick={()=>void refreshSession(session.id)}>Retry waiting recognition</button></div></div>}
