...
function recognitionInitial(r:RecognitionResult):Partial<WineInput>{return {producer:r.producer??'',wineName:r.wineName??'',vintage:r.vintage,recognizedProducer:r.recognizedProducer,recognizedWineName:r.recognizedWineName,recognizedVintageText:r.recognizedVintageText,vintageKind:r.vintageKind,releaseDesignation:r.releaseDesignation,country:r.country,region:r.region,appellation:r.appellation,recognizedRegion:r.recognizedRegion,recognizedAppellation:r.recognizedAppellation,grapes:r.grapes,grapeBlend:r.grapeBlend,wineStyle:r.style,alcoholPercentage:r.alcoholPercentage,sparklingDetails:r.sparklingDetails,tastingDate:r.tastingDate,locationName:r.locationName,latitude:r.latitude,longitude:r.longitude,tags:derivedTags({country:r.country,region:r.region,appellation:r.appellation,grapes:r.grapes,style:r.style}),recognitionConfidence:r.confidence,recognitionStatus:'review'}}
...
export function BatchScanPage(){
  ...
  // A wine saved from a batch links back to /batch-scan?session=<id>, so the
  // page reopens the batch it came from rather than whichever one happens to be
  // resumable. Read once: the effect below runs on mount only, and re-reading a
  // param the user never changes would just re-open a batch they navigated away
  // from.
  ...
  /**
   * Returning to the list is the right moment to re-read it. Polling only
   * follows the open session, so another batch that finished recognition while
   * the reader was working in here would otherwise still show its old status.
   * A failed re-read is not worth surfacing: the batch just worked on has
   * already been folded into the list from its own session payload.
   */
  async function leaveSession(){
    stopPoll();setSession(null);setReviewId(null);setLightbox(null);setDrafts([emptyWine(),emptyWine()]);setNotice('');
    try{setHistory((await listBatchSessions()).items)}catch{/* the open batch's row is already current */}
  }
  async function afterSaved(){if(!session)return;setReviewId(null);setLightbox(null);const next=await refreshSession(session.id),nextReady=next.items.find(x=>x.status==='ready');if(nextReady)setReviewId(nextReady.id);setNotice('Wine confirmed and saved. The original photo is attached to the wine.')}
  ...
  if(session?.status==='uploading'){
    ...
    return <section className="batch-scan-page"><PageHeader title="Batch scan" subtitle={checkingPending?'Checking this device for the remaining photos…':pendingLocal>0?`${pendingLocal} wine${pendingLocal===1?' is':'s are'} still stored on this device. Resume to finish the upload before background recognition starts.`:complete?'All wines reached the server, but the batch was not queued. You can finish submission now.':expected?'The upload was interrupted and the remaining files are no longer available in this browser. Cancel this batch and start again.':'This older incomplete batch was never submitted. Its missing original files cannot be recovered automatically; cancel it and start again.'}/>
      <div className="batch-upload-panel"><div className="batch-upload-progress"><strong>UPLOAD INCOMPLETE</strong><span>{expected?`${session.totalItems} / ${expected}`:`${session.totalItems} staged`}</span></div>{expected>0&&<div className="batch-progress-track"><span style={{width:`${Math.min(100,(session.totalItems/expected)*100)}%`}}/></div>}<small>Recognition has not started yet. WineLog is only safe to leave after this status changes to QUEUED or RUNNING.</small><div className="batch-upload-actions">{canResume&&<button type="button" disabled={submitting||cancelling} onClick={()=>void resumeUpload()}>{submitting?'Resuming…':pendingLocal>0?'Resume upload':'Finish & queue'}</button>}<button type="button" className="secondary-danger" disabled={cancelling} onClick={()=>void removeBatch(session.id,session.status,session.confirmedItems)}>{cancelling?'Cancelling…':'Cancel batch'}</button></div></div>
      {error&&<p className="scan-error">{error}</p>}{notice&&<p className="producer-notice">{notice}</p>}
    </section>;
  }

  if(session)return <section className="batch-scan-page"><PageHeader title="Batch scan" stats={[`${session.totalItems} wine${session.totalItems===1?'':'s'} in this batch`]} subtitle={waitingItems.length?`${waitingItems.length} waiting for recognition · ${readyItems.length} ready for review · ${session.confirmedItems} confirmed.`:session.status==='queued'||session.status==='running'?'WineLog is processing these wines in the background. You can close WineLog safely.':`${readyItems.length} ready for review · ${session.confirmedItems} confirmed.`}/>
    <div className={`batch-session-status ${session.status}`}><strong>{session.status.toUpperCase()}</strong><span>Support ID {session.id} · expires {new Date(session.expiresAt).toLocaleDateString()}</span></div>{notice&&<p className="producer-notice">{notice}</p>}{error&&<p className="scan-error">{error}</p>}
    {waitingItems.length>0&&['queued','running','ready','partial','failed'].includes(session.status)&&<div className="batch-upload-panel"><div className="batch-upload-progress"><strong>WAITING FOR RECOGNITION</strong><span>{waitingItems.length} waiting</span></div><small>If recognition appears to have stopped, retry is safe: WineLog first checks existing work and only requeues a request when needed.</small><div className="batch-upload-actions"><button type="button" disabled={recovering||retryLockedSessionId===session.id} onClick={()=>void retryWaitingRecognition()}>{recovering?'Retrying…':retryLockedSessionId===session.id?'Retry queued…':'Retry waiting recognition'}</button></div></div>}
    <div className="batch-result-grid">{session.items.map((item,index)=>{const alt=item.recognition?.wineName?`${item.recognition.wineName} label`:`Wine ${index+1}`;return <article className={`batch-result-card ${item.status}`} key={item.id}>{item.imageIds[0]?<BatchImage id={item.imageIds[0]} alt={alt} onOpen={item.recognition?src=>setLightbox({src,alt}):undefined}/>:<div className="batch-image-placeholder">Wine {index+1}</div>}<div><span className="batch-item-state">{item.status}</span><h2>{item.saved?.wineName||item.recognition?.wineName||`Wine ${index+1}`}</h2><p>{item.saved?.producer||item.recognition?.producer||(item.status==='failed'?'Recognition did not complete.':'Waiting for recognition')}</p>{item.status==='failed'&&<small>Support ID {item.id}</small>}{item.recognition&&<small>{(item.saved?item.saved.vintage:item.recognition.vintage)?`${item.saved?item.saved.vintage:item.recognition.vintage} · `:''}{Math.round(item.recognition.confidence*100)}% confidence</small>}{item.status==='ready'&&<button type="button" onClick={()=>setReviewId(item.id)}>Review & save</button>}{item.status==='failed'&&<button type="button" className="secondary-danger" onClick={()=>void discard(item)}>Discard</button>}{item.status==='confirmed'&&item.confirmedWineId&&<Link to={`/wines/${item.confirmedWineId}`} state={linkFrom({to:`/batch-scan?session=${session.id}`,label:'Batch scan'})}>View saved wine</Link>}</div></article>})}</div>
    {reviewItem?.recognition&&<div className="batch-review"><div className="batch-review-heading"><div><p className="eyebrow">REVIEW IDENTIFICATION</p><h2>{reviewItem.recognition.wineName||'Review wine'}</h2></div><button type="button" onClick={()=>{setReviewId(null);setLightbox(null)}}>Close</button></div><div className="batch-review-images">{reviewItem.imageIds.map((id,i)=>{const alt=`${reviewItem.recognition?.wineName||'Wine'} label ${i+1}`;return <BatchImage key={id} id={id} alt={alt} onOpen={src=>setLightbox({src,alt})}/>})}</div>{!memberView&&<small>Tap a label photo to enlarge it and verify the identification against the original.</small>}<WineForm initial={recognitionInitial(reviewItem.recognition)} onSave={wine=>confirmBatchWine(session.id,reviewItem.id,wine)} onSaved={()=>void afterSaved()} submitLabel="Confirm & save wine" enableFriendTagging/><button type="button" className="secondary-danger batch-discard" onClick={()=>void discard(reviewItem)}>Discard this identification</button></div>}
    <div className="batch-footer-actions"><button type="button" onClick={()=>void leaveSession()}>Start another batch</button>{!['queued','running'].includes(session.status)&&<button type="button" className="secondary-danger" disabled={cancelling} onClick={()=>void removeBatch(session.id,session.status,session.confirmedItems)}>{cancelling?'Removing…':'Remove batch'}</button>}</div>
    {lightbox&&<ImageLightbox src={lightbox.src} alt={lightbox.alt} onClose={()=>setLightbox(null)}/>} 
  </section>;

  const phaseLabel=uploadProgress?.phase==='persisting'?'Preparing resumable upload':uploadProgress?.phase==='queueing'?'Queueing for recognition':'Uploading photos';
  return <section className="batch-scan-page"><PageHeader title="Batch scan" subtitle="Each section is one bottle. Add front, back, neck or other labels inside that bottle’s section. WineLog keeps each bottle’s labels together and identifies the wines in the background."/>{error&&<p className="scan-error">{error}</p>}{notice&&<p className="producer-notice">{notice}</p>}
    ...
    <button type="button" className="batch-add-wine" disabled={submitting} onClick={()=>setDrafts(xs=>[...xs,emptyWine()])}>＋ Add another wine</button><div className="batch-submit-panel"><p>WineLog prepares the selected photos automatically for efficient background recognition.</p>{uploadProgress&&<div className="batch-upload-progress-card"><div><strong>{phaseLabel}</strong><span>{uploadProgress.uploaded} / {uploadProgress.total} wines uploaded</span></div><div className="batch-progress-track"><span style={{width:`${uploadProgress.total?Math.min(100,(uploadProgress.uploaded/uploadProgress.total)*100):0}%`}}/></div><small>{uploadProgress.phase==='queueing'?'All files reached WineLog. Queueing background recognition now…':uploadProgress.resumable?'Keep WineLog open for the fastest upload. If you leave this page, you can resume the remaining wines on this device.':'Keep WineLog open until uploading completes; resumable browser storage is unavailable.'}</small></div>}<div className="batch-submit-actions"><button type="button" className="wide-action primary" disabled={!readyToSubmit||submitting} onClick={()=>void submit()}>{submitting?phaseLabel:`Identify ${populated.length||0} wines in batch`}</button>{uploadProgress&&<button type="button" className="secondary-danger" disabled={cancelling} onClick={()=>void removeBatch(uploadProgress.sessionId,'uploading')}>{cancelling?'Cancelling…':'Cancel batch'}</button>}</div></div>
