...
export function BatchScanPage(){
  ...
  if(session)return <section className="batch-scan-page"><div className="hero compact"><p className="eyebrow">BATCH SCAN</p><h1>{session.totalItems} wines in this batch.</h1><p>{session.status==='queued'||session.status==='running'?'Gemini is processing these independent wine requests in the background. You can close WineLog safely.':`${readyItems.length} ready for review · ${session.confirmedItems} confirmed.`}</p></div>
    ...
    <div className="batch-result-grid">{session.items.map((item,index)=>{const alt=item.recognition?.wineName?`${item.recognition.wineName} label`:`Wine ${index+1}`;return <article className={`batch-result-card ${item.status}`} key={item.id}>{item.imageIds[0]?<BatchImage id={item.imageIds[0]} alt={alt} onOpen={item.recognition?src=>setLightbox({src,alt}):undefined}/>:<div className="batch-image-placeholder">Wine {index+1}</div>}<div><span className="batch-item-state">{item.status}</span><h2>{item.recognition?.wineName||`Wine ${index+1}`}</h2><p>{item.recognition?.producer||item.error||'Waiting for recognition'}</p>{item.recognition?.vintage&&<small>{item.recognition.vintage} · {Math.round((item.recognition.confidence??0)*100)}% confidence</small>}{item.status==='ready'&&<button type="button" onClick={()=>setReviewId(item.id)}>Review & save</button>}{item.status==='failed'&&<button type="button" className="secondary-danger" onClick={()=>void discard(item)}>Discard</button>}{item.status==='confirmed'&&item.confirmedWineId&&<a href={`/wines/${item.confirmedWineId}`}>View saved wine</a>}</div></article>})}</div>
