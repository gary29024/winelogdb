...
export async function deleteGroupScanSession(id:string){await read<{ok:true}>(await fetch(`/api/group-recognition/sessions/${encodeURIComponent(id)}`,{method:'DELETE',headers:authHeaders()}),'Could not remove Group Photo history');knownSessions.delete(id);forgetGroupSessionImages(id)}
