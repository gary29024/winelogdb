...
async function read<T>(response:Response,message:string):Promise<T>{
  const body=await response.json().catch(()=>({})) as ApiError&T;
  if(!response.ok)throw new Error(typeof body.error==='string'?body.error:`${message} (${response.status})`);
  return body as T;
}
async function fetchFile(url:string,name:string){
  const response=await fetch(url,{headers:authHeaders(),cache:'no-store'});if(!response.ok)throw new Error(`Could not restore Group Photo image (${response.status})`);
  const blob=await response.blob();return new File([blob],name,{type:blob.type||'application/octet-stream',lastModified:Date.now()});
}
...
async function updateState(session:GroupScanStoredSession){
  await read<ServerSession>(await fetch(`/api/group-recognition/sessions/${encodeURIComponent(session.id)}`,{method:'PUT',headers:authHeaders(true),body:JSON.stringify(statePayload(session))}),'Could not update Group Photo history');
}
async function createServerSession(session:GroupScanStoredSession){
  ...
  const cropKeys:Array<{key:string;width:number;height:number}>=[];
  for(const item of session.items){if(!item.crop)continue;fd.append('crops',item.crop.file);cropKeys.push({key:item.key,width:item.crop.width,height:item.crop.height})}
  fd.append('cropKeys',JSON.stringify(cropKeys));
...
export async function saveGroupScanSession(session:GroupScanStoredSession){
  if(knownSessions.has(session.id)){await updateState(session);return}
  const pending=creating.get(session.id);if(pending){await pending;await updateState(session);return}
  const task=createServerSession(session).finally(()=>creating.delete(session.id));creating.set(session.id,task);await task;
}
export async function getGroupScanSession(id:string):Promise<GroupScanStoredSession|null>{
  const response=await fetch(`/api/group-recognition/sessions/${encodeURIComponent(id)}`,{headers:authHeaders(),cache:'no-store'});if(response.status===404)return null;
  const session=await read<ServerSession>(response,'Could not load Group Photo history');knownSessions.add(id);
  const [photo,recognitionPhoto]=await Promise.all([
    fetchFile(`/api/group-recognition/sessions/${encodeURIComponent(id)}/image/original`,'group-photo-source'),
    fetchFile(`/api/group-recognition/sessions/${encodeURIComponent(id)}/image/preview`,'group-photo-preview.jpg')
  ]);
  const items=await Promise.all(session.items.map(async item=>{
    const crop=item.hasCrop?await fetchFile(`/api/group-recognition/sessions/${encodeURIComponent(id)}/items/${encodeURIComponent(item.key)}/crop`,'group-wine.jpg'):null;
    return {key:item.key,recognition:item.recognition,crop:crop?{file:crop,metadata:session.metadata,width:item.cropWidth,height:item.cropHeight}:null,savedId:item.savedId,removed:item.removed,manual:item.manual} satisfies GroupScanStoredItem;
  }));
...
export async function listGroupScanSessions():Promise<GroupScanHistoryItem[]>{
  const result=await read<{items:GroupScanHistoryItem[]}>(await fetch('/api/group-recognition/sessions',{headers:authHeaders(),cache:'no-store'}),'Could not load Group Photo history');for(const item of result.items)knownSessions.add(item.id);return result.items;
}
