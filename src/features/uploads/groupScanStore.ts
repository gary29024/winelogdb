...
import { forgetGroupSessionImages,groupCropImageUrl,groupOriginalImageUrl,groupPreviewImageUrl,loadGroupImageBlob,rememberGroupImageBlob } from './groupImageCache';
...
async function fetchFile(url:string,name:string){const blob=await loadGroupImageBlob(url);return new File([blob],name,{type:blob.type||'application/octet-stream',lastModified:Date.now()})}
...
async function updateState(session:GroupScanStoredSession){await read<ServerSession>(await apiFetch(`/api/group-recognition/sessions/${encodeURIComponent(session.id)}`,{method:'PUT',headers:authHeaders(true),body:JSON.stringify(statePayload(session))}),'Could not update Group Photo history')}
async function createServerSession(session:GroupScanStoredSession){
  ...
  await read<ServerSession>(await apiFetch(`/api/group-recognition/sessions/${encodeURIComponent(session.id)}`,{method:'PUT',headers:authHeaders(),body:fd}),'Could not save Group Photo history');knownSessions.add(session.id);
  // These are the exact bytes just accepted by the server. Seed the shared
  // memory cache so opening source context or resuming on this device does not
  // immediately read the same R2 objects back again.
  rememberGroupImageBlob(groupOriginalImageUrl(session.id),session.photo);
  rememberGroupImageBlob(groupPreviewImageUrl(session.id),session.recognitionPhoto);
  for(const item of session.items){if(item.crop)rememberGroupImageBlob(groupCropImageUrl(session.id,item.key),item.crop.file)}
}
...
export async function getGroupScanSession(id:string):Promise<GroupScanStoredSession|null>{
  const response=await apiFetch(`/api/group-recognition/sessions/${encodeURIComponent(id)}`,{headers:authHeaders(),cache:'no-store'});if(response.status===404)return null;const session=await read<ServerSession>(response,'Could not load Group Photo history');knownSessions.add(id);
  const [photo,recognitionPhoto]=await Promise.all([fetchFile(groupOriginalImageUrl(id),'group-photo-source'),fetchFile(groupPreviewImageUrl(id),'group-photo-preview.jpg')]);
  const items=await Promise.all(session.items.map(async item=>{const crop=item.hasCrop?await fetchFile(groupCropImageUrl(id,item.key),'group-wine.jpg'):null;return {key:item.key,recognition:item.recognition,crop:crop?{file:crop,metadata:session.metadata,width:item.cropWidth,height:item.cropHeight}:null,savedId:item.savedId,removed:item.removed,manual:item.manual} satisfies GroupScanStoredItem}));
...
export async function listGroupScanSessions():Promise<GroupScanHistoryItem[]>{const result=await read<{items:GroupScanHistoryItem[]}>(await apiFetch('/api/group-recognition/sessions',{headers:authHeaders(),cache:'no-store'}),'Could not load Group Photo history');for(const item of result.items)knownSessions.add(item.id);return result.items}
export async function deleteGroupScanSession(id:string){await read<{ok:true}>(await apiFetch(`/api/group-recognition/sessions/${encodeURIComponent(id)}`,{method:'DELETE',headers:authHeaders()}),'Could not remove Group Photo history');knownSessions.delete(id);forgetGroupSessionImages(id)}
export async function linkGroupScanWine(sessionId:string,itemKey:string,wineId:string){const pending=creating.get(sessionId);if(pending)await pending;return read<{ok:true;wineId:string;sessionId:string;clientKey:string}>(await apiFetch(`/api/group-recognition/sessions/${encodeURIComponent(sessionId)}/items/${encodeURIComponent(itemKey)}/link`,{method:'POST',headers:authHeaders(true),body:JSON.stringify({wineId})}),'Wine was saved, but its Group Photo source could not be linked')}
export async function fetchGroupSourcePreview(sessionId:string){return fetchFile(groupPreviewImageUrl(sessionId),'group-photo-source.jpg')}
