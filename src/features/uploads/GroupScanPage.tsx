...
export function GroupScanPage(){
  ...
  async function markSaved(key:string,id:string){
    setItems(current=>current.map(item=>item.key===key?{...item,savedId:id}:item));setActiveKey(null);
    if(!sessionId)return;
    try{await linkGroupScanWine(sessionId,key,id);setNotice('Wine saved. Its bottle crop is the wine photo and the original Group Photo is retained as secondary source context.');await refreshHistory()}catch(e){setError((e as Error).message||'Wine saved, but its Group Photo source could not be linked')}
  }
