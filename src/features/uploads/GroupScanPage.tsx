...
export function GroupScanPage(){
  ...
  return <section className="group-scan-page">
    ...
    {!photo?<div className="photo-source-card group-photo-source"><div className="scan-mark"><AppIcon kind="group-photo"/></div><h2>Choose one group photo</h2><p>Best results come from a clear lineup where labels are reasonably visible. Duplicate bottles of the same wine are logged once.</p><button type="button" className="scan-button" onClick={()=>input.current?.click()}>Choose group photo</button><input ref={input} className="visually-hidden" type="file" accept="image/*" onChange={e=>void choose(e.target.files?.[0])}/></div>:
