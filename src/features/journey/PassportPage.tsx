  ...
  return <section className="journey-page passport-page">
    ...
    <div className="passport-pair-grid passport-secondary-grid">
      <section className="passport-mini-card">
        ...
        {recent.length?<div className="passport-recent-list">{recent.slice(0,2).map(item=><Link to={item.shared?`/shared/${item.id}`:`/wines/${item.id}`} state={linkFrom({to:'/',label:'Passport'})} key={item.id}>
          ...
          <div className="passport-recent-copy"><strong>{item.wineName}{item.vintage?` ${item.vintage}`:''}</strong><small>{[item.producer,item.region||item.country,item.shared&&item.sharedBy?`Shared by ${item.sharedBy}`:''].filter(Boolean).join(' · ')}</small></div>
